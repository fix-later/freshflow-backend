...
/// <summary>
/// <see cref="IAssistantChatClient"/> implementation backed by GLM 5.2 (Free) via the ZenMux
/// gateway. ZenMux is OpenAI-compatible, so this wraps the official OpenAI SDK's
/// <see cref="OpenAI.Chat.ChatClient"/> (via Microsoft.Extensions.AI's <c>AsIChatClient()</c>)
/// pointed at <see cref="ZenMuxOptions.BaseUrl"/> instead of api.openai.com. The orchestrator only
/// ever sees <see cref="IAssistantChatClient"/> — no OpenAI/ZenMux type leaks past this class.
/// </summary>
public sealed class ZenMuxChatClient : IAssistantChatClient
{
    private readonly IChatClient _inner;
    private readonly ZenMuxOptions _options;

    /// <summary>Production constructor — builds the OpenAI-compatible client pointed at ZenMux.</summary>
    public ZenMuxChatClient(IOptions<ZenMuxOptions> options, HttpClient httpClient)
    ...
    internal ZenMuxChatClient(ZenMuxOptions options, IChatClient inner)
    {
        _options = options;
        _inner = inner;
    }

    private static IChatClient BuildInnerClient(ZenMuxOptions options, HttpClient httpClient)
    {
