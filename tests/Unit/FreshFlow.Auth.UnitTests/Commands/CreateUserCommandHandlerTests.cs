...
public sealed class CreateUserCommandHandlerTests
{
    ...
    public async Task Handle_RestaurantRole_CreatesRestaurantProfile()
    {
        ...
        _restaurants.CreateAsync(Arg.Any<Guid>(), "Pho Ba Tu", null, null, null, default).Returns(Guid.NewGuid());

        var result = await _sut.Handle(
            new CreateUserCommand("r@test.com", "P@ss1", "restaurant", null, "Pho Ba Tu"), default);

        result.IsSuccess.Should().BeTrue();
        await _restaurants.Received(1).CreateAsync(Arg.Any<Guid>(), "Pho Ba Tu", null, null, null, default);
    }
