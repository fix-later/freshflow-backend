...
| T012 | Implement Admin user creation and restaurant approval | Auth | Implement `POST /api/v1/admin/users` (Admin role required). `AuthService.CreateUserAsync` validates email uniqueness, hashes the password, creates the `users` row with the specified role, assigns `market_agent` users to the requested market, and creates `restaurant` profiles with `is_approved = false`. Return HTTP 201 with the new `userId`. Implement `PATCH /api/v1/admin/restaurants/{restaurantId}/approve` for Admin approval flow. Public self-registration is not exposed in v1. | `src/FreshFlow.API/Controllers/AdminController.cs`, `src/Modules/Auth/FreshFlow.Auth.Application/Commands/CreateUserCommand.cs`, `src/Modules/Auth/FreshFlow.Auth.Application/Commands/ApproveRestaurantCommand.cs`, `src/Modules/Auth/FreshFlow.Auth.Application/DTOs/CreateUserRequestDto.cs`, `src/Modules/Auth/FreshFlow.Auth.Application/Validators/CreateUserRequestValidator.cs`, `src/Modules/Auth/FreshFlow.Auth.Application/Services/AuthService.cs`, `src/Modules/Auth/FreshFlow.Auth.Domain/Entities/User.cs` | T009 | M | 1. `POST /api/v1/admin/users` by an Admin with role `MARKET_AGENT`, `HUB_STAFF`, `DRIVER`, or `RESTAURANT` creates the account and returns HTTP 201 with `userId`. 2. A non-Admin calling `POST /api/v1/admin/users` receives HTTP 403. 3. Creating a user with a duplicate email returns HTTP 409 with error code `EMAIL_ALREADY_EXISTS`. 4. `PATCH /api/v1/admin/restaurants/{restaurantId}/approve` sets `is_approved = true`. |

---

### Group C — Pricing Module (T013–T018)
...
| T026 | Implement order grouping + manual auto-batch trigger | Orders | Implement `POST /api/v1/admin/order-groups` (Admin manual adjustment), `GET /api/v1/admin/order-groups/{id}`, and `POST /api/v1/admin/order-groups/auto-batch`. Create `OrderBatchingService` shared by the 22:00 cutoff job and the manual trigger. It groups eligible `CONFIRMED` orders by delivery zone and source market, creates `OrderGroup`/Procurement Batch records, transitions included orders to `BATCHED`, skips already-batched orders idempotently, and supports `targetDate`, `dryRun`, and `force` request fields. | `src/FreshFlow.API/Controllers/AdminController.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/Commands/CreateOrderGroupCommand.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/Commands/AutoBatchOrdersCommand.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/DTOs/CreateOrderGroupRequestDto.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/DTOs/AutoBatchOrdersRequestDto.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/DTOs/AutoBatchOrdersResponseDto.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/DTOs/OrderGroupDto.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/Services/OrderBatchingService.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/Services/OrderGroupService.cs`, `src/Modules/Orders/FreshFlow.Orders.Application/BackgroundJobs/DailyOrderBatchingJob.cs`, `src/Modules/Orders/FreshFlow.Orders.Infrastructure/Persistence/Repositories/OrderGroupRepository.cs`, `src/Modules/Orders/FreshFlow.Orders.Domain/Repositories/IOrderGroupRepository.cs` | T025 | M | 1. `POST /api/v1/admin/order-groups` with a list of confirmed `orderIds` returns HTTP 201 with `orderGroupId`. 2. `POST /api/v1/admin/order-groups/auto-batch` with `dryRun=true` returns preview batches without writing DB rows. 3. `POST /api/v1/admin/order-groups/auto-batch` with `dryRun=false` creates batches, transitions included orders to `BATCHED`, and returns created batch count, batched order count, and skipped/conflict lists. 4. Re-running auto-batch for the same target date skips already-batched orders and does not create duplicate active groups. |
...
| T046 | Implement pricing dashboard with SignalR live price feed | Frontend-Web | Implement the `PricingDashboardComponent` (in `features/pricing/`) displaying a live price board for a selected market. On load, call `GET /markets/{id}/products`. Subscribe to `PriceUpdated` SignalR events via `SignalrService`. On event receipt, update the relevant row in the component's price table reactively using the `async` pipe and `OnPush` change detection. | `freshflow-web/src/app/features/pricing/pricing-dashboard.component.ts`, `freshflow-web/src/app/features/pricing/pricing-dashboard.component.html`, `freshflow-web/src/app/features/pricing/pricing.service.ts` | T045, T017 | M | 1. The pricing dashboard displays real-time price updates without page refresh when a Kiosk Staff updates a price. 2. Updated rows show the latest price/quantity within 500 ms of the event receipt. 3. All components use `OnPush` change detection and the `async` pipe; no manual `subscribe` calls in component code. |
...
| 7 | **T013** — Price snapshots migration | T014, T015, T016, T018 | Pricing tables must exist before any pricing logic |
| 8 | **T015** — Market products listing | T016, T022 | Price update and order creation read current prices |
| 9 | **T016** — Price update endpoint | T017 | Broadcast trigger starts here |
| 10 | **T017** — PricingHub broadcast | T046, T049 | SignalR price feed is the core real-time value proposition |
...
| **Phase 7 — Pricing Enhancements** | T018 | T016, T017 | 1 day |
...
| `OrderBatchingService` | `Orders/OrderBatchingServiceTests.cs` | Dry-run preview, grouping by delivery zone and source market, idempotent skip of already-batched orders, manual trigger uses same behavior as scheduled cutoff job |
