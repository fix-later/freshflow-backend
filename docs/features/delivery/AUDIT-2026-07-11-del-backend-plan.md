...
| **SCRUM-325** | UC-DEL-08+09 | Sync Delivery Status | = realtime DeliveryStopUpdated (FR-LOG-007 AC3 + FR-NOT-003) | ✅ implemented local — chờ review/commit |
...
### SCRUM-323 — Report Delivery Issue (UC-DEL-07) — ⬜ PLANNED (prompt codex sẵn sàng)
Driver báo vấn đề khi giao (không giao được/hư hỏng/khách từ chối...). **Chốt DEC-DEL-12 (leader 2026-07-12, khảo sát code THẬT):**
- **Bảng MỚI `delivery_issues` do Logistics sở hữu — TỪ CHỐI tái dùng `OrderIssue`.** Verify code: `OrderIssue` (Orders.Domain) = Orders-owned, reporter = **restaurant** (RBAC restaurant, `ReportOrderIssueCommandHandler` check `restaurantReader.FindByUserIdAsync == order.RestaurantId`), chỉ cho đơn `Delivered`, type `Missing/Wrong/Damaged`. Sai actor (323 = **driver**) + tái dùng ⇒ Logistics phải cross-module-write bảng Orders (vi phạm isolation xuyên epic). ⇒ bảng riêng Logistics.
- **Độc lập `deliveries.failure_reason`** (option b main): báo issue là action riêng, KHÔNG ép mark FAILED, KHÔNG đụng luồng `UpdateDeliveryStatus` (319). Không ràng buộc delivery status (chỉ IDOR).
- **CRUD nội bộ Logistics thuần — KHÔNG integration event / realtime / notify** (ranh giới: realtime/notify = SCRUM-325). Nếu sau cần notify issue cho ops → follow-up 325.
- Entity `DeliveryIssue` plain sealed class (mẫu `Delivery`): `id, delivery_id (FK deliveries), issue_type (undeliverable/damaged/customer_rejected/other + CHECK), description (≤1000), status (open/resolved default open, KHÔNG resolve endpoint), reported_by (driver Guid), timestamps, deleted_at`. Endpoint `POST /api/v1/driver/deliveries/{deliveryId}/issues` (DriverController, RBAC driver, IDOR delivery→route→driver 403). Repo `IDeliveryIssueRepository` + EF config snake_case + migration create-only `AddDeliveryIssues`. Prompt `codex-prompt-scrum323.md`.

### SCRUM-325 — Sync Delivery Status (UC-DEL-08+09) — ✅ IMPLEMENTED LOCAL (chờ review)
= FR-LOG-007 AC3 `DeliveryStopUpdated` SignalR tới `restaurant:{id}` + FR-NOT-003 (DeliveryStarted/Completed persist). **Chốt DEC-DEL-13 (leader 2026-07-12, khảo sát code THẬT):**
- **Event MỚI `DeliveryStopUpdatedIntegrationEvent(OrderId, RouteId, DeliveryId, Status, OccurredAt)`** (Contracts), publish từ `UpdateDeliveryStatusCommandHandler` (319) cho CẢ 3 nhánh arrived/delivered/failed — **THÊM 1 dòng publish** vào handler 319 đã commit (có chủ đích). `DeliveryStarted`/`DeliveryCompleted` giữ nguyên.
- **Realtime = Logistics** (KHÔNG tái dùng `OrderHub` — module Orders, cross-module cấm). Build `DeliveryHub` (`Logistics.Infrastructure/Realtime`, map `/hubs/delivery`) + `IDeliveryBroadcastService` (interface Application/Abstractions, impl Infrastructure/Realtime) đúng CLAUDE.md. 2 handler broadcast (Logistics.Application/EventHandlers) consume `DeliveryStarted` + `DeliveryStopUpdated` → `IOrderStatusReader.FindByIdAsync` (ĐÃ trả `RestaurantId` — không cần seam mới) → group `restaurant:{id}` + `admin:delivery`. KHÔNG broadcast từ `DeliveryCompleted` (trùng StopUpdated=delivered).
- **Persist = Notifications** (mẫu NOT): thêm `NotificationType.delivery_update` (**KHÔNG migration** — cột `type` varchar(50) HasConversion<string>, không CHECK); 2 consumer `DeliveryStarted`/`DeliveryCompleted` → `INotificationRecipientResolver.ResolveUserIdByOrderIdAsync` (seam MỚI order→restaurant→user) → `WriteAsync`. KHÔNG persist StopUpdated (FR-NOT-003 chỉ started/completed).
- Hub group-join cần user→restaurant → seam Logistics `IRestaurantOwnerReader.FindRestaurantIdByUserIdAsync` (mẫu `RestaurantCoordinateReader`).
- **Giới hạn (note, KHÔNG chặn):** Redis backplane CHƯA cấu hình (`AddSignalR()` trần) → single-node MVP; JWT-in-query hub negotiate chưa có (Order/Pricing cùng gap) — follow-up. Offline-sync = client queue+replay, backend idempotent nhờ guard status 319.
...
- **2026-07-12 (SCRUM-323 + SCRUM-325 plan — 2 task cuối DEL)** — Khảo sát code THẬT: (a) `OrderIssue` Orders-owned reporter=restaurant/Delivered-only ⇒ TỪ CHỐI reuse, bảng riêng `delivery_issues` Logistics (**DEC-DEL-12**); 323 = CRUD thuần, KHÔNG event/realtime, độc lập `failure_reason`. (b) SignalR: chỉ `PricingHub`+`OrderHub` map, KHÔNG `DeliveryHub`, `AddSignalR()` trần (không Redis backplane) ⇒ build DeliveryHub single-node (**DEC-DEL-13**); `IOrderStatusReader` ĐÃ trả `RestaurantId` (không cần seam broadcast mới); `NotificationType` = varchar(50) HasConversion không CHECK ⇒ thêm `delivery_update` KHÔNG migration; 319 `UpdateDeliveryStatusCommandHandler` ĐÃ publish `DeliveryCompleted`+`route.Complete()` (committed) — 325 THÊM 1 publish `DeliveryStopUpdatedIntegrationEvent` (event mới) cho cả 3 nhánh. Ranh giới 323↔325: 323 lưu record, 325 realtime/notify (issue-event = follow-up optional). 2 prompt codex soạn xong (`codex-prompt-scrum323.md`/`325.md`), gửi main. KHÔNG code, KHÔNG commit.
