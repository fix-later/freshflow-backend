...
### SCRUM-321 — Capture Proof of Delivery (UC-DEL-06) — ⬜ MỚI
Driver chụp ảnh/chữ ký khi giao xong. **Tái dùng Cloudinary signed-upload** (SCRUM-363: `ICloudinarySignatureService`, mẫu `CreateProductImageUploadSignatureCommandHandler`) — BE cấp signature, client upload thẳng Cloudinary, lưu `proof_url` (+ optional signature) lên `deliveries` (thêm cột, cần migration nhỏ) hoặc bảng POD riêng. `docs/03` KHÔNG có cột proof → thêm mới. Gắn với DELIVERED (POD thường là điều kiện/kèm mark-delivered). Cần chốt: POD bắt buộc trước khi DELIVERED hay ghi nhận song song.
