...
| Notify Statement (in-app + email) | SCRUM-269 | Net-new | ✅ Hoàn thành 2026-07-20 — notify khi sinh sao kê MỚI (in-app + email SMTP config-gated), Tier-1 due date mềm |
...
| 2026-07-20 | **SCRUM-269 ✅ hoàn thành (net-new).** Notify khi sao kê tháng được sinh MỚI (không notify khi idempotent re-generate). Publish `CreditStatementGeneratedIntegrationEvent` (Contracts) trực tiếp từ `CreditStatementGenerationService` sau commit thành công — không qua domain-event vì `CreditStatement` không phải AggregateRoot. Notifications consumer `CreditStatementGeneratedIntegrationEventHandler`: in-app (`INotificationWriter`, type mới `credit_statement`) + email; 2 kênh try/catch độc lập. Email: `IEmailSender` mới + `SmtpEmailSender` (System.Net.Mail, không thêm NuGet) config-gated `Notifications:Email:Smtp` + `LogEmailSender` fallback. Seam `NotificationRecipientRow` mở rộng join `users` lấy `Email` (đã confirm cột `users."Id"/"Email"/"DeletedAt"`). **Đề xuất hóa đơn = Tier 1** (sao kê + due date mềm = PeriodEnd + 15d dẫn xuất, KHÔNG lifecycle paid/overdue) vì công nợ là số dư chạy liên tục, settlement không gắn kỳ → per-statement status cần allocation chưa cần (YAGNI). Verify: build 0 lỗi, format sạch, Orders 510 + Notifications 129 GREEN, `has-pending-model-changes` no drift (seam không đổi snapshot). **Follow-up (chưa làm):** integration test real-Postgres cho seam join `users` (mọi cột đã verify riêng + SQLite fixture chứng minh SQL shape). Chưa commit. |
