...
| 2026-06-18 | Task #6/Phase 4 commit | **committed `ab0619d`** — `feat(orders): SCRUM-214 add order cancellation flow` (body ghi đủ SCRUM-214/UC-ORD-15, SCRUM-217/UC-ORD-16, SCRUM-220/UC-ORD-17). Sau commit code, working tree chỉ còn `src/FreshFlow.API/appsettings.Development.json` modified ngoài scope |
| 2026-06-18 | Task #7/Phase 5 (Recurring orders + generation job, UC-ORD-09/10/11) | **in_progress** — bắt đầu sau Task #6 docs commit `1a96edc`. Scope theo audit: tạo/quản lý scheduled recurring order (daily/weekly), list/detail/instances/update/pause-cancel cho restaurant owner/admin nếu cần, hosted service DB-only sinh concrete draft order instances từ `scheduled_orders` và xử lý missed-execution recovery. Redis reservation/job release vẫn deferred Task #10; không thêm schema mới nếu tận dụng được `scheduled_orders`/`orders.scheduled_order_id`. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #7/Phase 5 focused tests | **in_progress** — implement scheduled recurring order API/application + DB-only generation service/hosted service đã qua focused Orders unit test: `dotnet test tests/Unit/FreshFlow.Orders.UnitTests/FreshFlow.Orders.UnitTests.csproj --no-restore` pass **269/269**. Quyết định kỹ thuật trong scope: do schema hiện tại không có recurring line-item template, job sinh draft order instance rỗng gắn `scheduled_order_id`/`scheduledFor` để restaurant hoàn thiện item trước confirm; không thêm migration/schema mới ở Task #7. Đang review diff và chạy verification rộng. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #7/Phase 5 review hardening | **in_progress** — sau review T027/audit, bổ sung `MissedExecutionCount` + hosted-service log `MISSED_EXECUTION` khi job recover nhiều occurrence, thêm regression test idempotency chạy lại cùng window không tạo duplicate, thêm direct handler tests invalid recurrence. Focused Orders unit test pass **272/272**. Quyết định pause: schema chính thức `scheduled_orders` chỉ có `cancelled_at`, không có `paused_at`/status; để giữ đúng scope no-migration, Task #7 expose cancel/deactivate schedule và ghi nhận pause riêng là yêu cầu cần schema/API decision sau nếu supervisor yêu cầu. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #7/Phase 5 verdict | **reviewer PASS, ready to commit** — hoàn thành UC-ORD-09/10/11 DB-only: tạo scheduled recurring order daily/weekly cho restaurant approved, list/detail/update/cancel cho restaurant owner/admin, list generated instances, generation service + hosted service 60s tạo draft order instance gắn `scheduled_order_id`/`scheduledFor`, recover missed executions và log `MISSED_EXECUTION`, idempotency dựa trên `last_executed_at`. No migration/schema; do không có recurring line-item template, generated draft instance rỗng để restaurant thêm items trước confirm. Verification: `git diff --check` clean; Orders unit pass **272/272**; Auth unit pass **357/357**; Pricing unit pass **247/247**; Catalog unit pass **136/136**; Integration pass **105/105**; `dotnet format FreshFlow.slnx --verify-no-changes` pass; `dotnet ef migrations has-pending-model-changes --project src/FreshFlow.Infrastructure.Persistence --startup-project src/FreshFlow.API` no changes; `dotnet test FreshFlow.slnx --no-restore` exit 0. Warnings còn lại là existing package/SDK warnings (`NU1510`, EF Relational 10.0.4/10.0.9 conflict, EF tools 10.0.1 vs runtime 10.0.9, ASP/Scalar obsolete). `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên KHÔNG stage/commit |
| 2026-06-18 | Task #7/Phase 5 commit | **committed `427e739`** — `feat(orders): SCRUM-198 add recurring scheduled orders` (body ghi đủ SCRUM-198/UC-ORD-09, SCRUM-201/UC-ORD-10, SCRUM-204/UC-ORD-11). Sau commit code, working tree chỉ còn audit docs và `src/FreshFlow.API/appsettings.Development.json` modified ngoài scope |
| 2026-06-18 | Task #8/Phase 6 (Receipt + issue report + reorder, UC-ORD-18/19/21) | **in_progress** — bắt đầu sau Task #7 docs commit `7670f14`. Scope theo audit: `ConfirmReceipt` khi order đã `DELIVERED` và set `confirmed_receipt_at`; `ReportOrderIssue` tạo bảng mới `order_issues`; `ReOrderFromHistory` tạo draft order mới từ order cũ. Cần migration/schema mới theo §3.B. Redis/Realtime vẫn deferred theo Task #9/#10. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #8/Phase 6 focused tests | **in_progress** — đã implement domain/application/API skeleton cho UC-ORD-18/19/21: `Order.ConfirmReceipt`, `OrderIssue` + enums/repository/config, commands `ConfirmOrderReceipt`, `ReportOrderIssue`, `ReorderFromHistory`, endpoints `PATCH /orders/{orderId}/receipt`, `POST /orders/{orderId}/issues`, `POST /orders/{orderId}/reorder`. Migration mới `AddOrderReceiptIssues` thêm `orders.confirmed_receipt_at` + `order_issues` với FK tới `orders`, `order_items`, raw FK `reported_by -> users(\"Id\")`. Focused Orders unit test pass **307/307** trước final verification. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #8/Phase 6 verification progress | **in_progress** — `dotnet format FreshFlow.slnx --verify-no-changes` pass; `dotnet ef migrations has-pending-model-changes --project src/FreshFlow.Infrastructure.Persistence --startup-project src/FreshFlow.API` báo no pending model changes. Chỉ còn warning existing: EF tools 10.0.1 cũ hơn runtime 10.0.9. Đang chạy test rộng trước verdict/commit; `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #8/Phase 6 preliminary verification | **superseded by review hardening** — initial verification xanh trước khi phát hiện validator precedence issue: `git diff --check` clean; Orders unit pass **307/307**; Pricing unit pass **247/247**; Auth unit pass **361/361**; Catalog unit pass **136/136**; Integration pass **105/105**; `dotnet test FreshFlow.slnx --no-restore` exit 0; format pass; EF no pending model changes. Kết quả này bị superseded bởi dòng review hardening ngay sau đó, cần rerun sau fix trước commit |
| 2026-06-18 | Task #8/Phase 6 review hardening | **in_progress** — sau review cuối trước commit, phát hiện validator `ReportOrderIssueCommandValidator` đang bắt `AffectedQuantity <= 0` ở FluentValidation layer nên API có thể trả `400 VALIDATION_ERROR` thay vì handler code `INVALID_ISSUE_QUANTITY`/422. Fix: validator chỉ giữ structural ID checks, business validation issue type/quantity/description nằm trong handler; thêm regression handler test cho quantity 0/âm. Đang rerun verification sau fix; `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #8/Phase 6 verdict after hardening | **reviewer PASS, ready to commit** — hoàn thành UC-ORD-18/19/21 DB-only: confirm receipt chỉ cho order `DELIVERED` và set `confirmed_receipt_at`; report issue tạo `order_issues` với issue type/quantity/description/item ownership validation và giữ `INVALID_ISSUE_QUANTITY` ở handler layer (422); reorder tạo draft order mới từ order cũ bằng product snapshot/stock hiện tại. Migration `AddOrderReceiptIssues` thêm `orders.confirmed_receipt_at`, `order_issues`, FK tới `orders`/`order_items`, raw FK `reported_by -> users(\"Id\")`. Không làm Redis reservation/realtime/issue resolution workflow ngoài scope. Verification sau hardening: `git diff --check` clean; Orders unit pass **309/309**; Pricing unit pass **247/247**; Auth unit pass **361/361**; Catalog unit pass **136/136**; Integration pass **105/105**; `dotnet test FreshFlow.slnx --no-restore` exit 0; `dotnet format FreshFlow.slnx --verify-no-changes` pass; `dotnet ef migrations has-pending-model-changes --project src/FreshFlow.Infrastructure.Persistence --startup-project src/FreshFlow.API` no changes. Warnings còn lại là existing package/EF tool warnings. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên KHÔNG stage/commit |
| 2026-06-18 | Task #8/Phase 6 commit | **committed `54ff704`** — `feat(orders): SCRUM-222 add receipt and reorder flows` (body ghi đủ SCRUM-222/UC-ORD-18, SCRUM-225/UC-ORD-19, SCRUM-231/UC-ORD-21). Sau commit code, working tree chỉ còn audit docs và `src/FreshFlow.API/appsettings.Development.json` modified ngoài scope |
| 2026-06-18 | Task #9/Phase 7 Real-time OrderHub (UC-ORD-14, SCRUM-211) | **in_progress** — kích hoạt deferred task theo yêu cầu `continue` sau Task #8 docs commit `bdfa246`. Scope: `OrderHub`, `IOrderBroadcastService`, broadcast `OrderStatusChanged` sau commit tới `restaurant:{restaurantId}` (và `admin:orders` theo `docs/04-api-design.md`), map `/hubs/orders`, unit tests theo pattern PricingHub. Redis backplane/soft-reservation vẫn deferred Task #10; không thêm schema/migration |
| 2026-06-18 | Task #9/Phase 7 focused tests | **in_progress** — đã implement `IOrderBroadcastService`, `OrderStatusChangedBroadcastDto`, `OrderStatusChangedDomainEventHandler`, `OrderHub` auto-join (`restaurant:{restaurantId}`/`admin:orders`), `OrderBroadcastService`, route `/hubs/orders`, DI registration và tests realtime/event handler. Focused Orders unit test pass **327/327** sau restore project vì Orders unit csproj thêm `Microsoft.AspNetCore.App` framework reference. Không migration/schema; `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên không stage/commit |
| 2026-06-18 | Task #9/Phase 7 verdict | **reviewer PASS, ready to commit** — hoàn thành UC-ORD-14 realtime DB-committed broadcast: `OrderStatusChangedDomainEventHandler` chạy post-commit, map status sang API wire strings, `OrderBroadcastService` gửi `OrderStatusChanged` song song tới `restaurant:{restaurantId}` và `admin:orders`; `OrderHub` `[Authorize]` auto-join restaurant/admin/operations_manager theo JWT + `IRestaurantReader`; API map `/hubs/orders`. Không thêm schema/migration, không làm Redis backplane/soft-reservation (Task #10 vẫn deferred). Verification: `git diff --check` clean; Orders unit pass **327/327**; Pricing unit pass **247/247**; Auth unit pass **361/361**; Catalog unit pass **136/136**; Integration pass **105/105**; `dotnet test FreshFlow.slnx --no-restore` exit 0; `dotnet format FreshFlow.slnx --verify-no-changes` pass; `dotnet ef migrations has-pending-model-changes --project src/FreshFlow.Infrastructure.Persistence --startup-project src/FreshFlow.API` no changes. Warnings còn lại là existing package/EF tool warnings. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope nên KHÔNG stage/commit |
| 2026-06-18 | Task #9/Phase 7 commit | **committed `6eeff22`** — `feat(orders): SCRUM-211 add realtime order status hub`. Sau commit code, working tree chỉ còn audit docs và `src/FreshFlow.API/appsettings.Development.json` modified ngoài scope |
| 2026-06-18 | Codex full review Task #8/#9 | **finding MEDIUM** — Task #8 (receipt/issues/reorder) không phát hiện blocker mới. Task #9 có contract/security gap: `OrderHub` hiện `[Authorize]` chung nên role ngoài ma trận `docs/04-api-design.md` (`market_agent`, `hub_staff`, `driver`) vẫn negotiate/connect được `/hubs/orders`, dù không được join group. Cần harden `[Authorize(Roles = "admin,operations_manager,restaurant")]` và thêm regression test unauthorized role bị reject/không connect theo access matrix. Review verification: `dotnet test tests/Unit/FreshFlow.Orders.UnitTests/FreshFlow.Orders.UnitTests.csproj --no-restore --no-build` pass **327/327**. `src/FreshFlow.API/appsettings.Development.json` vẫn modified ngoài scope |
